...
def get_data(request: HttpRequest):
    try:
        ...
        # Pisahkan geometri menjadi dua bagian: satu untuk poligon (Polygon) dan satu lagi untuk garis (LINESTRING)
        gdf_polygon = gdf[gdf.geometry.geom_type == 'Polygon']
        gdf_linestring = gdf[gdf.geometry.geom_type == 'LineString']

        # Nama file shapefile untuk poligon dan garis
        shapefile_output_polygon = os.path.join(shapefile_output, f'polygon_{username}.shp')
        shapefile_output_linestring = os.path.join(shapefile_output, f"linestring_{username}.shp")
        
        # Simpan data GeoDataFrame masing-masing sebagai shapefile jika tidak kosong
        if not gdf_linestring.empty:
            gdf_linestring.to_file(shapefile_output_linestring, driver='ESRI Shapefile', encoding='utf-8')
        #     gdf_polygon.to_file(shapefile_output_polygon, driver='ESRI Shapefile', encoding='utf-8')
        # else :
        ...
        data = {
            ...
            # "user_dir": user_dir,
            "zip_file": zip_file,
            # "shapefile_output": shapefile_output,
            "code": 200,
